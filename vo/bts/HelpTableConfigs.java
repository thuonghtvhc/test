...
public class HelpTableConfigs {
	...
	public static String getColumns(List<CTableConfig> configList) {
		String constructor="var column = [{pinned: true, text: 'No.',datafield: '', columntype: 'number', filterable: false, exportable: false, cellsrenderer: numberrenderer, width: 35,align: 'center' }";
	...
	//Lay dieu kien tim kiem tren Grid
		...
		public static String getSeriesChar(List<String> series)
		{
			...
			for (String serie: series) {
				sierie =sierie + con+"{dataField:'" + serie.replace(" ", "_")+ "',displayText:'"+serie+"'}";
				con=",";
			}
			sierie = sierie + "]";
			return sierie;
		...
		// Tao grid
		public static String getGridPagingReportUrl(List<CTableConfig> configList, String tableID, String url,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight,String typeSelectRow,String groupColumn,String autoheigh) {
			String s = "";
		...
		// Tao grid BINH THUONG
				public static String getGridPagingUrl(List<CTableConfig> configList, String tableID, String url,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight,String typeSelectRow,String autoRowheigh,String groupHeader) {
					String s = "";
					if (typeSelectRow==null||typeSelectRow.equals(""))
					{
						typeSelectRow="singlerow";
					...
					s += "showfilterrow: true,";
					s += "filterable: true,";
					if (autoRowheigh!=null&& autoRowheigh.equals("Y"))
					{
						s += "autorowheight: true,";
						s += "autoheight: true,";
					}
					else if (autoRowheigh!=null&& !autoRowheigh.equals("Y"))
					{
						s += "		height: "+autoRowheigh+",";
					}
					s += "sortable: true,";
					s += "pageable: true,";
					s += "altrows: true,";
					...
					s += "pagesizeoptions: ['20', '100', '200', '300', '400', '500'],";
					s += "columns: column";
					s += " });";
					if (menu!=null && !menu.equals(""))
					{
		...
		public static String getGridReportDataString(List<CTableConfig> configList, String tableID, String data,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight) {
			String s = "";
		...
		/*tạo grid du lieu*/
		public static String getGridReportDataString(List<CTableConfig> configList, String tableID, String data,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight,String isShowaggregates,String groupColumn,String groupHeader, String editEnable) {
			String s = "";
			String sampleData = data;
			s+="$(document).ready(function () {";
			s+="    var theme = getDemoTheme();";
			...
			s+="		datafields: datafields," ;
			s+="        localdata: data,";
			if (editEnable!=null && editEnable.equals("Y")) {
				s+="        updaterow: function (rowid, rowdata, commit) {";
				s+="            commit(true);";
				s+="        },";
			}
			s+= "		pagenum: 0,";
			...
			s += "		selectionmode: 'multiplecellsadvanced',";
			...
			if (groupColumn!=null)
			{
			s += "		,groupable: true,groupsrenderer: groupsrenderer,groups: ["+groupColumn+"],groupsexpandedbydefault:true";
			}
			s += " });";
			// Create context menu
			s += "	var contextMenu = $('#"+menu+"').jqxMenu({ width: 200, autoOpenPopup: false, mode: 'popup', theme: getTheme() });";
		...
		public static String getGrouprender(List<CTableConfig> configList) {
			...
			constructor += "}";
			constructor += "var result='<div style=\"position: relative; height: 23px;color: red;font-weight: bold;\"><div style=\"float:left;\">' + text + '</div>'; ";
			for (CTableConfig config : configList) {
				if (config.getAggregates() !=null && !config.getAggregates().equals("")){
					constructor += " result= result+ '<div style=\" float: left;\" >, "+config.getColumnName()+": '+ aggregate"+config.getDataField()+".sum +'</div>';";
		        }
			}
		...
		public static String getGridUrl(List<CTableConfig> configList, String tableID, String url,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight,String typeSelectRow,String autoheigh,String autoRowheigh,String groupHeader,String isfilter, String ispaging, String colorGrid ) {
			String s = "";
			if (typeSelectRow==null||typeSelectRow.equals(""))
			{
				typeSelectRow="singlerow";
			...
			s += "   var numberrenderer = function (row, column, value) {";
			s += "        return '<div style=\"text-align: center; position: relative;top: 50%;transform: translateY(-50%);\">' + (1 + value) + '</div>';";
			s += "    };";
			...
			if (autoheigh!=null&& autoheigh.equals("Y"))
			{
				s += "autoheight: true,";
			}
			else if (autoheigh!=null&& !autoheigh.equals("Y"))
			{
				s += "height: "+autoheigh+",";
			...
			s += "scrollmode: 'logical',";
			/*s += "scrollmode: 'deferred',";
			s += "scrollfeedback: function(row)";
			s += "{";
			s += "var position = $('#" + tableID + "').jqxGrid('scrollposition');";
			s += "var left = position.left;";
			s += "var top = position.top;";
			s += "$('#" + tableID + "').jqxGrid('scrolloffset', top, left);";
			s += "},";*/
			s += "selectionmode: '"+typeSelectRow+"',";
			if (groupHeader!=null && !groupHeader.equals(""))
			{
		...
		// Tao grid BINH THUONG
		public static String getGridEditUrl(List<CTableConfig> configList, String tableID, String url,String listSourceId, List<CTableConfig> listSource, String menu, String hightlight,String enableEdit,String urlEdit,String autoRowheigh,String groupHeader) {
			String s = "";
            ...
            s += "         if (data==1) { alert(\"Chỉnh sửa thành công!\");commit(true);}" +
            		"else {alert(\"Chỉnh sửa không thành công!\");commit(true);} ";
            s += "     },";
            s += "     error: function(jqXHR, textStatus, errorThrown)";
            s += "     {";
            s += "         var msg = JSON.parse(jqXHR.responseText);";
            s += "         altert(msg.message);";
            s += "         commit(false);";
            s += "    }";
            s += " });";
			s += "},";
			//ADD NEW ROW
			/*s += "addrow: function (rowid, rowdata, position, commit)  {";
			s += " var data = $.param(rowdata);";
			s += " console.log(data);	";
			s += "  $.ajax({";
			s += "     dataType: 'json',";
